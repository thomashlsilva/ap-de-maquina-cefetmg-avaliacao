...
class Fold():
    ...
    def gerar_k_folds(df_dados,val_k:int,col_classe:str,num_repeticoes:int=1,seed:int=1,
                    num_folds_validacao:int=0,num_repeticoes_validacao:int=0) -> List["Fold"]:
        """
        ...
        #...o parametro val_k
        num_instances_per_partition = math.floor(len(df_dados)/(val_k))
        #folds de saida
        arr_folds = []
        
        #print("df_dados length :"+str(len(df_dados)))
        #print("num_instances_per_partition :"+str(num_instances_per_partition))
        #print("num_folds_validacao "+str(num_folds_validacao))
        #print("num_repeticoes_validacao "+str(num_repeticoes_validacao))
        ...
        for num_repeticao in range(num_repeticoes):
            ...
            seedI = seed+num_repeticao
            df_dados_rand = df_dados.sample(frac=1,random_state= seedI)

            #Impressão dos ids dos dados (exiba o print para testes)
            #print("Dados: "+str(len(df_dados_rand)))
            ...
            for num_fold in range(val_k):
                ...
                nl = '\n ==============================='
                print(f"# Fold: {num_fold}    Inicio: {ini_fold_to_predict} -  Fim: {fim_fold_to_predict} {nl}")
